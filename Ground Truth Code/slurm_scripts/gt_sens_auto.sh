...
#SBATCH --output=../../slurm_scripts/slurm_output/slurm-%j.out
#SBATCH --error=../../slurm_scripts/slurm_output/slurm-%j.err
...
#Current version: MLP: 1.1, CNN: 1.1, ResNet18: 1.1
version="$6"
#Create metadata file to store the parameters used for the sensitivity analysis
#touch "$csv_folder/metadata.csv"

#Add the header to the metadata file
...
do
    ...
    #echo "$num_epochs,$seed_value,$min_size,$RATE,$num_classes,$dataset,$model_architecture,$version" >> $csv_folder/metadata.csv
    #Pass in corrruption rate and number of epoch parameters
    python3 ../python_files/sensitivity_analysis.py --corruption_rate "$RATE" --dataset "$dataset" --model_architecture "$model_architecture" --num_classes "$num_classes" --base_folder "$base_folder" --data_chunk "$min_size" --epochs "$num_epochs" --jobid "$SLURM_JOBID" --seed "$seed_value" --manual "$isManual" --all_corruption "${CORRUPTION_RATES[@]}"  >> $txt_folder/network_output.txt 2>> $txt_folder/network_error.txt

    if [ $? -ne 0 ]; then
        success=1  # Update status to indicate failure
        deactivate
        exit $success
    fi
...
python3 ../../python_files/runCalcs.py --jobid "$SLURM_JOBID"
