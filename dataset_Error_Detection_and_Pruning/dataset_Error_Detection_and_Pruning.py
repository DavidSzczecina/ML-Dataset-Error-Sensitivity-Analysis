...
def save_mislabeled_images(dataset, label_issues, hdf5_path, output_dir, pred_probabilities):
    ...
    for rank, index in enumerate(label_issues, start=1):
        image, true_label, image_name = dataset[index]
        image = getImgData(image_name)
        
        # Decode the true label to get the actual word
        true_label_word = dataset.label_encoder.inverse_transform([true_label])[0]

        # Get predicted probability for this image
        predicted_prob = pred_probabilities[index]
        predicted_class = np.argmax(predicted_prob)
        predicted_label_word = dataset.label_encoder.inverse_transform([predicted_class])[0]  # Convert index to label
        certainty = predicted_prob[predicted_class] * 100  # Get the certainty percentage
        
        # Plot and save the image with the true label, predicted label, accuracy, and filename in the title
        plt.imshow(image)
        plt.title(f'Rank: {rank}, True: {true_label_word}, Predicted: {predicted_label_word}, Certainty: {certainty:.1f}%, Filename: {image_name}')
        plt.axis('off')
...
def evaluate_model(model, testloader, loss_function, device, label_encoder):
    ...
    with torch.no_grad():
        for inputs, targets, _ in testloader:
            ...
            for i in range(targets.size(0)):
                label = targets[i].item()  # Ensure it's a scalar value
                class_total[label] += 1
    ...
    # Calculate accuracy for each class and use class names from the label encoder
    class_accuracy = {}
    class_names_ordered = []
    for i, class_name in enumerate(label_encoder.classes_):
        if class_total[i] > 0:
            class_accuracy[class_name] = 100.0 * class_correct[i] / class_total[i]
        else:
            class_accuracy[class_name] = 0.0  # Handle cases where no samples for this class
        class_names_ordered.append(class_name)  # Append class names in order

    return avg_loss, accuracy, fold_pred_probabilities, fold_true_labels, class_accuracy, class_names_ordered
...
if __name__ == '__main__':
  ...
  parser.add_argument("--print_epcoh_stats", dest='print_epoch_stats', default=False, action='store_true',  help="Whether to print the per class accuracies for each epoch")
  ...
  for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset), 1):
    ...
    for epoch in range(0, num_epochs):
      ...
      num_classes = 16  # Define based on your dataset
      test_loss, test_accuracy, fold_pred_probabilities, fold_true_labels, class_accuracy, classes = evaluate_model(model, testloader, loss_function, device, dataset.label_encoder)
